...
{
public:
  m2::RectD const LimitRect(ref_ptr<OverlayHandle> const & handle) const
  {
    return handle->GetExtendedPixelRect(m_modelView);
  }
  m2::RectD const & GetRect() const { return GetExtendedScreenRect(); }
...
public:
  using HandlesCache = std::unordered_set<ref_ptr<OverlayHandle>, OverlayHasher>;
...
private:
  ...
  OverlayTraits m_traits;
  StaticQuadTree<ref_ptr<OverlayHandle>, OverlayTraits, 3> m_quadTree;
